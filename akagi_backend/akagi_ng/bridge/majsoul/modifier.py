...
def get_default_mod_settings_dict() -> dict[str, Any]:
    return {
        "enabled": True,
        "config": {
...
def load_mod_settings_dict() -> dict[str, Any]:
...
def save_mod_settings_dict(settings: dict[str, Any]) -> None:
...
def verify_mod_settings_dict(data: Any) -> bool:
    ...
    if "config" in data and not isinstance(data["config"], dict):
        return False
    if "resource" in data and not isinstance(data["resource"], dict):
        return False
    return True
...
class MajsoulModifier:
    def __init__(self) -> None:
        self.safe: dict[str, Any] = {"contract": ""}
    ...
    def _pick_key(self, data: dict[str, Any], snake: str, camel: str) -> str:
        if camel in data and snake not in data:
            return camel
        return snake

    def _pick_related_key(self, data: dict[str, Any], snake: str, camel: str, *, prefer_camel_if: str | None = None) -> str:
        if camel in data and snake not in data:
            return camel
    ...
    def _get_value(self, data: dict[str, Any], snake: str, camel: str, default: Any = None) -> Any:
        if snake in data:
            return data[snake]
        if camel in data:
            return data[camel]
    ...
    def process(self, liqi_message: dict, *, from_client: bool, raw_content: bytes, liqi_proto: LiqiProto) -> PacketMutation:
    ...
    def _handle_notify(self, method: str, data: dict[str, Any], liqi_proto: LiqiProto) -> PacketMutation:
        match method:
            case ".lq.NotifyAccountUpdate":
                ...
                if isinstance(character_update, dict):
                    ...
                    if main_character:
                        main_character[self._pick_key(main_character, "character_id", "characterId")] = self.settings["config"][
                            "character"
                        ]
            ...
            case ".lq.NotifyRoomPlayerUpdate":
                for player in data.get("player_list", []):
                    self._apply_player_mods(
                        player,
                        own_account=self._get_value(player, "account_id", "accountId") == self.safe.get("account_id"),
                    )
    ...
    def _handle_request(self, method: str, data: dict[str, Any], msg_id: int, liqi_proto: LiqiProto) -> PacketMutation:
        ...
        match method:
            case ".lq.Lobby.changeMainCharacter":
                ...
                fake = True
                injects.append(self._build_character_update_notify(char_id, self._get_character_skin(char_id), liqi_proto))
            ...
            case ".lq.Lobby.loginBeat":
                self.safe["contract"] = data.get("contract", "")
            case ".lq.FastTest.authGame":
                self.safe["account_id"] = self._get_value(data, "account_id", "accountId")
        ...
        if fake:
            ...
            payload = liqi_proto.build_packet(
                MsgType.Req,
                ".lq.Lobby.loginBeat",
                {"contract": self.safe.get("contract", "")},
                msg_id=msg_id,
            )
    ...
    def _handle_response(self, method: str, data: dict[str, Any], msg_id: int, liqi_proto: LiqiProto) -> PacketMutation:
        modify = False
        ...
        match method:
            ...
            case ".lq.FastTest.authGame":
                ...
                logger.info(
                    ...
                    f"account_id={self.safe.get('account_id')}"
                )
                modify = True
            ...
            case ".lq.Lobby.fetchAccountInfo":
                ...
                if self._get_value(account, "account_id", "accountId") == self.safe.get("account_id"):
            ...
            case ".lq.Lobby.fetchAllCommonViews":
                ...
                data["views"] = [{"index": int(index), "values": values} for index, values in self.settings["config"]["views"].items()]
                modify = True
            ...
            case ".lq.Lobby.fetchAccountInfoExtra":
                if self._patch_composite_account_payload(data):
                    logger.info(
                        f"[MajsoulMod] Patched .lq.Lobby.fetchAccountInfoExtra keys={sorted(data.keys())}"
                    )
                    modify = True
            ...
            case ".lq.Lobby.fetchGameRecord":
                for account in data.get("head", {}).get("accounts", []):
                    self._apply_player_mods(
                        account,
                        own_account=self._get_value(account, "account_id", "accountId") == self.safe.get("account_id"),
                        allow_random=True,
    ...
    def _capture_login_state(self, data: dict[str, Any]) -> None:
        self.safe["account_id"] = self._get_value(data, "account_id", "accountId")
        account = data.get("account", {})
        self.safe["nickname"] = account.get("nickname", "")
        self.safe["title"] = account.get("title", 0)
        self.safe["loading_image"] = self._get_value(account, "loading_image", "loadingImage", [])

    def _patch_character_info(self, character_info: dict[str, Any]) -> None:
        ...
        self.safe["main_character_id"] = self._get_value(
            character_info, "main_character_id", "mainCharacterId", self.settings["config"]["character"]
        )
        self.safe["characters"] = character_info.get("characters", [])
    ...
    def _patch_composite_account_payload(self, data: dict[str, Any]) -> bool:
        modified = False
        ...
        if all_common_views_key in data:
            data[all_common_views_key] = {
                ...
                "views": [{"index": int(index), "values": values} for index, values in self.settings["config"]["views"].items()],
            }
            modified = True
    ...
    def _patch_room_players(self, room: dict[str, Any]) -> bool:
    ...
    def _patch_game_restore(self, data: dict[str, Any]) -> bool:
        ...
        modified = False
        for player in snapshot.get("account_views", []):
            self._apply_player_mods(
                player,
                own_account=self._get_value(player, "account_id", "accountId") == self.safe.get("account_id"),
                allow_random=True,
        ...
        for robot in snapshot.get("robot_views", []):
            self._upgrade_character(self._get_value(robot, "character", "character", {}))
            if self.settings["config"]["safe_mode"]:
                self._force_safe_mode(robot)
            modified = True
    ...
    def _cache_room_state_from_room(self, room: dict[str, Any]) -> None:
        if isinstance(room, dict):
            self.safe["room_state"] = copy.deepcopy(room)

    def _cache_room_state_from_update(self, update: dict[str, Any]) -> None:
        if isinstance(update, dict):
            self.safe["room_state"] = {
                "owner_id": update.get("owner_id", 0),
                "robot_count": update.get("robot_count", 0),
                "persons": copy.deepcopy(update.get("player_list", [])),
                "robots": copy.deepcopy(update.get("robots", [])),
                "positions": copy.deepcopy(update.get("positions", [])),
                "seq": update.get("seq", 0),
            }

    def _build_room_player_update_notify(self, liqi_proto: LiqiProto) -> bytes | None:
        room_state = self.safe.get("room_state")
    ...
    def _patch_login_account(self, account: dict[str, Any]) -> None:
        config = self.settings["config"]
    ...
    def _patch_character_update(self, character_update: dict[str, Any]) -> None:
        character_info = {
    ...
    def _patch_bag(self, bag: dict[str, Any]) -> None:
    ...
    def _inject_announcement(self, data: dict[str, Any]) -> None:
        announcement = {
    ...
    def _apply_player_mods(
        ...
        *,
        ...
        if own_account:
            ...
            if frame:
                player[self._pick_related_key(player, "avatar_frame", "avatarFrame", prefer_camel_if="avatarId")] = frame
    ...
    def _upgrade_character(self, character: dict[str, Any]) -> None:
        character["exp"] = 0
        character["level"] = 5
    ...
    def _force_safe_mode(self, player: dict[str, Any]) -> None:
    ...
    def _prefix_server(self, player: dict[str, Any]) -> None:
                    ...
                    "characters": [
                        {
                            ...
                            "level": 5,
                            "rewarded_level": [1, 2, 3, 4, 5],
                            "extra_emoji": self.catalog.emoji.get(char_id, []) if self.settings["config"]["emoji"] else [],
                        }
                    ]
                }
            }
    ...
    def _rebuild_notify(self, method: str, data: dict[str, Any], liqi_proto: LiqiProto) -> PacketMutation:
    ...
    def _current_views(self) -> list[dict[str, Any]]:
        return self.settings["config"]["views"].get(str(self._views_index()), [])

    def _resolved_views(self) -> list[dict[str, Any]]:
    ...
    def _current_avatar_frame(self, views: list[dict[str, Any]] | None = None) -> int | None:
