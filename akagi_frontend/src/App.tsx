...
function AppContent({ settingsPromise, theme, setTheme }: AppContentProps) {
  // Hooks
  ...
  const [locale, setLocale] = useState<string>(initialSettings.locale || 'zh-CN');
...
export default function App() {
  ...
  const settingsPromise = useMemo(async () => {
    const minDelay = new Promise((resolve) => setTimeout(resolve, 3000));
    const [settings] = await Promise.all([fetchSettingsApi(apiBase), minDelay]);
    return settings;
  }, [apiBase]);

  return (
    <Suspense fallback={<LaunchScreen />}>
      <AppContent settingsPromise={settingsPromise} theme={theme} setTheme={setTheme} />
    </Suspense>
  );
}
