...
async function saveMajsoulModSettingsApi(settings: MajsoulModSettings): Promise<MajsoulModSettings> {
  return fetchJson<MajsoulModSettings>('/api/majsoul-mod-settings', {
    method: 'POST',
...
export const MajsoulModSection: FC<Props> = memo(({ open }) => {
  ...
  const updateRoot = <K extends keyof MajsoulModSettings>(key: K, value: MajsoulModSettings[K]) => {
    if (!settings) return;
    void save({
      ...settings,
      [key]: value,
    ...
    <div className='space-y-4 rounded-2xl border border-white/8 p-5'>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
        <SettingsItem label={t('settings.majsoul_mod.enable_mod')} layout='row'>
          <CapsuleSwitch checked={settings.enabled} onCheckedChange={(checked) => updateRoot('enabled', checked)} />
        </SettingsItem>
        <SettingsItem
        ...
        >
          <CapsuleSwitch checked={settings.config.safe_mode} onCheckedChange={(checked) => update('safe_mode', checked)} />
        </SettingsItem>
        <SettingsItem label={t('settings.majsoul_mod.nickname')} description={t('settings.majsoul_mod.nickname_desc')}>
          <Input value={settings.config.nickname} onChange={(e) => update('nickname', e.target.value)} />
        </SettingsItem>
        <SettingsItem
        ...
        >
          <CapsuleSwitch checked={settings.config.show_server} onCheckedChange={(checked) => update('show_server', checked)} />
        </SettingsItem>
        <SettingsItem
        ...
        >
          <CapsuleSwitch checked={settings.config.emoji} onCheckedChange={(checked) => update('emoji', checked)} />
        </SettingsItem>
        <SettingsItem
