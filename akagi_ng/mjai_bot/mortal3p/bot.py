            ...
            if is_riichi_relevant(self.engine, self.player_id, e, is_3p=True):
                ...
                if is_reach_candidate:
                    ...
                    try:
                        ...
                        # Replay history
                        logger.info(f"Riichi Lookahead (3p): Replaying {len(self.history)} events.")
                        ...
                        logger.info("Riichi Lookahead (3p): Applying generic REACH event.")
                        # Parse the response from the simulation bot!
                        sim_resp = sim_bot.react(json.dumps(reach_event, separators=(",", ":")))

                        # Extract the simulation metadata from the RESPONSE, not the engine internal state
                        sim_data = json.loads(sim_resp)
                        sim_meta = sim_data.get("meta", {})

                        # Store simulation result in a special field
                        # We use the full sim_meta which contains q_values etc.
